...
def set_serial_port_baud_rate(ser: serial.Serial, baud_rate: int = 256000) -> bool:
    ...
    if baud_rate not in possible_baud_rates:
        raise ValueError(
            'The baud rate must be one of the following: 9600, 19200, 38400, 57600, 115200, 230400, 256000, 460800')
...
def set_zone_filtering(ser: serial.Serial,
    ...
    command_value = bytes.fromhex(
        f'{zone_filtering_mode:04x} {region1_x1:04x} {region1_y1:04x} {region1_x2:04x} {region1_y2:04x} {region2_x1:04x} {region2_y1:04x} {region2_x2:04x} {region2_y2:04x} {region3_x1:04x} {region3_y1:04x} {region3_x2:04x} {region3_y2:04x}')
